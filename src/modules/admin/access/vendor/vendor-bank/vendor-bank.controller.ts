...
import { VENDOR-BANK_FILTER_FIELDS, VendorBankService } from './vendor-bank.service';
import {
...
export class VendorBankController {
  ...
  @ApiFields(VENDOR-BANK_FILTER_FIELDS)
  @Permissions(
  ...
  @Get()
  public getVendorBanks(@PaginationParams() pagination: PaginationRequest): Promise<PaginationResponseDto<VendorBankResponseDto>> {
    return this.vendorBankService.list<VendorBankEntity, VendorBankResponseDto>(pagination);
  }
  ...
  @Get('/:id')
  public getVendorBankById(@Param('id', ParseIntPipe) id: number): Promise<VendorBankResponseDto> {
  ...
  public createVendorBank(
    ...
    return this.vendorBankService.createVendorBank({ ...dto,  createdBy: user.id });
  }
